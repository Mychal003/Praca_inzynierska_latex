...
Niniejszy rozdział przedstawia użyte metryki oraz wyniki ewaluacji zaimplementowanego systemu RAG. Eksperymenty zostały przeprowadzone lokalnie i potwierdzają skuteczności systemu.
...
Należy zauważyć, że klasyfikacja pytań w powyższej tabeli opiera się na decyzjach podjętych przez autonomiczny moduł \textbf{Query Classifier}~\ref{lst:classifier} w trakcie działania systemu, a nie na statycznym podziale ze zbioru danych. 
...
\end{figure}
Rysunek~\ref{fig:metrics_heatmap} przedstawia mapę ciepła (heatmap) zestawiającą metryki automatyczne z ocenami sędziego LLM.
...
\begin{enumerate}
    ...
    \item \textbf{Zachowanie wysokiego ugruntowania} -- wbrew obawom, złagodzenie zasad nie spowodowało wzrostu halucynacji (Groundedness wzrosło z 0.880 do 1.000).
\end{enumerate}
