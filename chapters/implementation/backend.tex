...
Struktura katalogów backendu przedstawia się następująco:
...
.3 rag\_pipeline.py.
.3 pdf\_processor.py.
.3 query\_classifier.py.
.3 conversation\_manager.py.
.2 vectorstores/.
.2 uploads/.
}
...
funkcjonalne. Tabela~\ref{tab:endpoints} przedstawia pełną listę 
...
Listing~\ref{lst:models} przedstawia definicje modeli SQLAlchemy.
...
class Message(db.Model):
    ...
    role = db.Column(db.String(20))  # 'user' lub 'assistant'
...
\begin{itemize}
    \item \texttt{User} $\rightarrow$ \texttt{Conversation} -- jeden użytkownik 
    może mieć wiele konwersacji
    \item \texttt{Conversation} $\rightarrow$ \texttt{Message} -- jedna konwersacja 
    zawiera wiele wiadomości (pytania i odpowiedzi)
\end{itemize}

\newpage
\subsection{System autoryzacji}

Autoryzacja oparta jest na tokenach JWT (JSON Web Token). 
Listing~\ref{lst:auth} przedstawia dekorator sprawdzający autoryzację.
...
Token JWT zawiera identyfikator użytkownika oraz czas wygaśnięcia 
(domyślnie 24 godziny). Dekorator \texttt{@token\_required} jest 
stosowany do wszystkich chronionych endpointów.
...
Vector store jest zapisywany na dysku w katalogu \texttt{vectorstores/}, 
